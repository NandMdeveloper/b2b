<?php
    ...
    for ($i=0; $i<sizeof($arr_dp); $i++){
        $query = "SELECT * FROM saStockAlmacen WHERE co_alma = 'P1' AND tipo = 'ACT' AND co_art = '".$arr_dp[$i]['co_art']."'";
        $res=sqlsrv_query($conn,$query);
        $row=sqlsrv_fetch_array($res);
        $cantidad = $arr_dp[$i]['total_art'];
    ...
    if($ban!=1){
        ...
        $sql="INSERT INTO saPedidoVenta (doc_num,descrip,co_cli,co_tran,co_mone,co_ven,co_cond,fec_emis,fec_venc,fec_reg,anulado,status,total_bruto,monto_imp,
          ...
          VALUES ('".$id_profit."','".$arr_pedidos[0]['doc_num']."-".$arr_pedidos[0]['descrip']."','".$arr_pedidos[0]['co_cli']."','00001','BSF','".$arr_pedidos[0]['co_ven']."','".$arr_pedidos[0]['co_cond']."','".$fechaL."',
        ...
        if($stmt){
            ...
            $sql2="INSERT INTO saPedidoVentaReng (reng_num,doc_num,co_art,des_art,co_alma,total_art,stotal_art,co_uni,co_precio,prec_vta,monto_desc,
              ...
              '".$arr_dp[$i]["co_precio"]."',".$arr_dp[$i]["prec_vta"].",".$arr_dp[$i]["monto_desc"].",".$impuesto.",'".$arr_pedidos[0]["OrderNumberTax"]."',0,0,".$arr_dp[$i]["monto_imp"].",
