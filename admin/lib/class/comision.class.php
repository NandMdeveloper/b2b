<?php
  class comision {
    ...
    public  $mescorto = array('',
            'Ene','Feb','Mar','Abr',
            'May','Jun','Jul','Ago',
            'Sep','Oct','Nov','Dic'

    ); // 
  public  $inicioVentas = '2017-09-01'; // 
    ...
    public function asignarZona($campos) {
      ...
      return $msn;
       ...
       public function getauditoriaParametros() {
        
        $conn = $this->getConMYSQL(); 
        $sel ="SELECT * FROM `cmslog` WHERE `detalle` LIKE '%#parametro%' 
 ...
 public function eliminarparametros($campos){
   ...
   if ($usuario=="javier" OR $usuario=="njose") { 
    ...
    public function getCalculoNCR_NDB($doc_num,$monto,$tipo,$desde,$hasta) {
       ...
       */
        ...
        if ($tipoNCR=='DEVO') {
            ...
            $sel ="select distinct 'Devolucion' as tipo, dc.num_doc as nro_orig,dc.tipo_doc as doc_orig,
                    fv.doc_num,fv.co_cli,cli.cli_des,cli.co_seg,fv.co_ven,fv.total_bruto as total_bruto_ori
                     from saDevolucionClienteReng as dc 
                    inner join saFacturaVenta as fv on dc.num_doc=fv.doc_num
                    inner join saCliente  as cli on fv.co_cli=cli.co_cli
                    where dc.num_doc in('".$doc_num."')";
     ...
     public function getParametrosEdicion($desde,$hasta) {
        ...
        $sel_r ="SELECT * FROM `parametros` 
         where   desde >= '".$desde."'  and  hasta   <=  '".$hasta."' and estatus = 1";    
    ...
    public function getHistoricounoXMes() {
       $conn = $this->getConMYSQL() ;
      $sql="SELECT DATE_FORMAT(periodo,'%m %Y'),count(periodo)as documentos, periodo  FROM `cmshistorialuno` where month(periodo) group by month(periodo)";
      $conn = $this->getConMYSQL() ;
        $rs_r = mysqli_query($conn,$sql) or die(mysqli_error($sql));
        while($linea=mysqli_fetch_array($rs_r)) {
      ...
      ?>
        <div class="col-lg-2 col-xs-6">
          <div class="small-box bg-aqua">
            <div class="inner">
              <h3><?php echo $linea[1] ?> <small><i class="fa fa-files-o" aria-hidden="true"></i>
              </small></h3>
              <p> <?php echo  $this->mescorto[$mes] ?> <?php echo $fecha[1] ?></p>
            </div>
            <a href="comisioneshistorico.php?periodo=<?php echo $linea[2] ?>" class="small-box-footer">
  ...
  public function getHistoricodetalles($mes) {
      $conn = $this->getConMYSQL();
      $sql="SELECT * FROM  cmshistorialuno  where  month(periodo) = month('".$mes."')";
      //var_dump($conn);

      $rs_r = mysqli_query($conn,$sql) or die(mysqli_error($conn));
      $x=0;
      $documentos = array();

      while($linea=mysqli_fetch_array($rs_r)) {
        foreach($linea as $key=>$value) {
          $documentos[$x][$key]=$value;
        }
        $x++;
      }
  ...
  public function getParametrosCortes($desde,$hasta) {
    ...
    while($linea=mysqli_fetch_array($rsb)) {
     ...
     public function getParametrosCorteseditar($desde,$hasta) {
      ...
      $c= 1;
       ...
       <div class="alert alert-dismissible alert-info ">
        ...
        <span class='pull-right'> 
          ...
          <a href='comisonparametroseditar.php?desde=<?php echo $corte['desde']; ?>&hasta=<?php echo $corte['hasta']; ?>'>
    ...
    public function getParametros($desde,$hasta){
        ...
        for($x=0; $x < 12 ; $x++) { 
            ...
            where month(desde)=  ".$mes." and YEAR(desde)= ".$anio." and estatus=1 group by desde";
            ...
            if ($can > 1) {
                 ...
                 /* BUSCAMOS CADA CORTE Y LO INTEGRAMOS EN UN ARRAY PARA AJUNTAR PARAMETROS*/
                    ...
                    while($row=mysqli_fetch_array($rsb)) {
                        ...
                        $sel_r ="SELECT * FROM `parametros` 
                        where  desde  >='".$r_desde."' and  hasta  <= '".$r_hasta."'  and estatus=1";    
                ...
                where MONTH(desde) >='".$mes."' and MONTH(hasta) <= '".$mes."'   and estatus=1
                and YEAR(desde)=".$anio."";    
    ...
    public function calculoBasico2($datos,$parametros,$ventas) {
       // var_dump($parametros[1]['id']); exit();
        $segClave = $this->segmentoClave;

        /* COMISION TRADICIONALES */
        $segTradicional = $this->segmentoTradicional;
        $segDetal = $this->segmentoDetal;
        ...
        if (isset($parametros) and count($parametros)) {
            $saldoCero = $parametros[0];
        if (floatval($datos['saldo_factura']) <= $saldoCero['limite1']) {
            ...
            for ($i=0; $i < count($parametros) ; $i++) { 
                ...
                /* CUENTAS TRADICIONAL  - DETAL */
                if (trim($datos['co_seg']) ==  $segTradicional or trim($datos['co_seg']) ==  $segDetal ) { 
                    $condicion = 'Tradicional';
                }

                if (isset($parametros[$i]['limite1']) and isset($parametros[$i]['limite2'])  and isset($parametros[$i]['cuenta']) ){

                    if ($datos['diascalle']  >=  $parametros[$i]['limite1']
                        and  $datos['diascalle'] <=  $parametros[$i]['limite2']
                        and $parametros[$i]['cuenta'] == $condicion and $ventas == null
                        ) {
                            if ($parametros[$i]['tipo']=='Comision' and $comision['cal_comision'] == 0 and $ventas == null) {
                                $comision['comision'] = $this->porcentaje($datos['total_bruto'],$parametros[$i]['porcentaje']);
                                $comision['porcentaje'] = $parametros[$i]['porcentaje'];
                                $comision['cal_comision'] = $parametros[$i]['id'];
                            }
                            if ($parametros[$i]['tipo']=='Reserva' and $comision['cal_reserva'] == 0 and $ventas == null) {
                                $comision['reserva'] = $this->porcentaje($datos['total_bruto'],$parametros[$i]['porcentaje']);
                                $comision['porcentajeR'] = $parametros[$i]['porcentaje'];                            
                                $comision['cal_reserva'] = $parametros[$i]['id'];
                            }
                        } 

                   //VENTAS COMISIONES
                    if ($ventas == "ventas" and $parametros[$i]['cuenta'] == $condicion ) {
                            if ($parametros[$i]['tipo']=='Ventas' 
                              and  $datos['cneg'] >= $parametros[$i]['limite1'] and  $datos['cneg'] <= $parametros[$i]['limite2']
                              and $comision['cal_comision'] == 0) {
                                $comision['comision'] = $this->porcentaje($datos['total_bruto'],$parametros[$i]['porcentaje']);
                                $comision['porcentaje'] = $parametros[$i]['porcentaje'];
                                $comision['cal_comision'] = $parametros[$i]['id'];
                            }
    ...
    //public function calculoBasico($co_ven,$condicion,$saldo_factura,$diasCalle,$total_bruto,$fVencimiento,$fcobro,$cneg) {
    public function calculoBasico($datos,$parametros) {
         $segClave = $this->segmentoClave;
   ...
   public function buscar($array,$campo1,$campo2) {
     ...
     */         
    public function listadoFacturaComisionVentas($desde,$hasta) {
        $sql = "select 
              fv.doc_num,
              fv.co_ven as covendedor,
              ven.ven_des,
              fv.co_cli,
              cli.cli_des,
              fv.total_bruto,
              ...
              fv.fec_emis,
               fv.fec_venc,
               DATEDIFF(DAY, fv.fec_emis, fv.fec_venc) as dias,
              fv.co_cond,
              cp.cond_des,
              cp.dias_cred,
              '' as factura
                ...
                if ($cann == 1) {
                    ...
                    for ($l=0; $l <  $cann  ; $l++) {    
                      /* comparamos fecha */
                        $fecha_desde = new DateTime($lista_parametros[$mes_doc]['datos'][$l]['desde']);
                        $fecha_hasta = new DateTime($lista_parametros[$mes_doc]['datos'][$l]['hasta']);
                        $fecha_emision = new DateTime($row['fec_emis']->format("Y-m-d"));
         ...
         while($row=sqlsrv_fetch_array($resulta)) {
            ...
            foreach($row as $key=>$value) {   
              ...
              if ($key == 'total_bruto' or $key == 'total_neto' or  $key == 'monto_imp' ) {
                ...
                $notascr[$i][$key] = $value * -1;

              } else {
                $notascr[$i][$key] = $value;
              }

              $notascr[$i]['comision']= $comisionCal;
    ...
    END ) * ( CASE WHEN DC.anulado = 1 THEN 0
        ...
        $bsql ="SELECT
              ...
              TP.descrip,
                        ...
                        ven.ven_des as vendedor,
                        ZN.zon_des as zona, P.co_seg,seg.seg_des
    ...
    $sel_venc ="
      SELECT
         ...
         $t = 0;
          while($row=sqlsrv_fetch_array($result)) {          
              foreach($row as $key=>$value) {
                ...
                $facturas_vencidas[$t]['idParametro']=0;
                $facturas_vencidas[$t]['editada']=0;
        ...
        /* BUSCAR FACTURAS EDITADAS MANUALMENTE */
        $editadas = $this->getFacturasEditadasIndividualmente();
       /* FACTURAS QUE ES ESTAN CON FECHA DE RECEPCION */
        for ($i=0; $i < count($pedidos) ; $i++) { 
           $nfacturas[] = $pedidos[$i]['factura'];
        }

       /*  DOCUMENTO BUSCAR FACTURAS EDITADAS MANUALMENTE */
         $nfacturaEditadas = array();
        for ($g=0; $g < count($editadas) ; $g++) { 
           $nfacturaEditadas[] = $editadas[$g]['documento'];
        }
        ...
        */    
        ...
        for ($j=0; $j < count($facturas_vencidas) ; $j++) {             
            ...
            $idRegistroFacura = array_search(trim((int)$facturas_vencidas[$j]['nro_orig']),$nfacturas);             
           ...
           if (!empty($idRegistroFacura)) {
            ...
            $hasta_limite = new DateTime($hasta);
            $entro = "NO";
     ...
     /* PARAMETROS ARBITRARIOS SI UNA FECHA NO COICIDE CON LOS PARAMETROS CARGADOS*/
        ...
        $parametros = array();            
          ...
          if (trim($facturas[$i]['co_tipo_doc']) == "ADEL" or trim($facturas[$i]['co_tipo_doc']) == "AJNA"
            ...
            if (trim($facturas[$i]['co_tipo_doc']) == 'FACT') {
                ...
                /* BUSCAMOS EN LA PRE CARGA SI EXISTE LA FACTURA EDITADA INDIVIDUALMENTE */
                $idEditada = "N";   
                $idEditada = array_search(trim($facturas[$i]['nro_orig']),$nfacturaEditadas);   

                //vector para guardar datos SI tiene la factura una edicion
                $nuevosDatos = array();

                if (is_numeric($idEditada)) {
                  $facturas[$i]['editada']=1;
                  //SE BUSCA EL CALCULO ARBITRARIO
                  $nuevosDatos = $editadas[$idEditada];
                 
                }
            
                /* BUSCAMOS EN LA PRE CARGA SI EXISTE LA FACTURA CON FECHA RECIBIDO */
                $idRegistroFacura = array_search(trim((int)$facturas[$i]['nro_orig']),$nfacturas);             
                if (!empty($idRegistroFacura) or count($nuevosDatos) > 0) {
                      ...
                      /* SE BUSCA ULTIM FECHA DE COBRO */            
                       ...
                       if (!empty($nfcobro)) {
                        ...
                        if (count($nuevosDatos) > 0) {
                          if ($nuevosDatos['cobro'] != '0000-00-00') {
                            $nfcobro = $nuevosDatos['cobro'];
                           $fcobro = date_format(date_create($nfcobro),'d/m/Y');
                          }
                        }         
                        ...
                        if (!empty($fcobro) and !empty($nfVcto) ) {
                            //$fecha1 = new DateTime($facturaRecibido['fecha_recibido']);
                            ...
                            $diascalle =  $diferencia + $facturas[$i]['dias_cred'];                        
                        }
                    ...
                    if ($facturas[$i]['saldo'] <= $saldoCero['limite1'] and $idvend == FALSE) {
                       ...
                       if (count($nuevosDatos) > 0) {
                         if ($nuevosDatos['comision'] > 0) {
                            $facturas[$i]['porcentaje'] = $nuevosDatos['comision'];
                            $facturas[$i]['comision']=$this->porcentaje($datos['total_bruto'],$nuevosDatos['comision']);
                         }
      ...
      for ($b=0; $b < $cant; $b++) { 
       ...
       if ($facturas[$b]['co_tipo_doc'] == "N/CR" and $facturas[$b]['fec_venc'] != 0) {
        ...
        if (trim($facturas[$b]['co_tipo_doc']) == "FACT") {
            $doc_fac[] = $facturas[$b];
        }
        if (trim($facturas[$b]['co_tipo_doc']) == "N/CR") {
            $doc_ncr[] = $facturas[$b];
        }

        if (trim($facturas[$b]['co_tipo_doc']) == "N/DB") {
            $doc_ndb[] = $facturas[$b];
        }

        if (trim($facturas[$b]['co_tipo_doc']) != "FACT" 
            and trim($facturas[$b]['co_tipo_doc']) != "N/CR"
            and trim($facturas[$b]['co_tipo_doc']) != "N/DB") {
            $doc_res[] = $facturas[$b];
...
{ 
    ...
    for ($x=0; $x < count($factura) ; $x++) {
       ...
       if ((trim($factura[$x]['co_tipo_doc']) == 'FACT' or trim($factura[$x]['co_tipo_doc']) == 'N/CR' or trim($factura[$x]['co_tipo_doc']) == 'N/DB')
         and $factura[$x]['comision'] != 'X') {          
          ...
          if (!empty(trim($factura[$x]['fecha_despacho'])) and strlen($factura[$x]['fecha_despacho'])==10) {
            $fecha_despacho = trim($factura[$x]['fecha_despacho']);
            $f = explode("/", $fecha_despacho);
            $fecha_despacho = $f[2]."-".$f[1]."-".$f[0];
          }

          if (!empty(trim($factura[$x]['fecha_recibido'])) and strlen($factura[$x]['fecha_recibido'])==10) {
            $fecha_recibido = trim($factura[$x]['fecha_recibido']);
            $f = explode("/", $fecha_recibido);
            $fecha_recibido = $f[2]."-".$f[1]."-".$f[0];
          }
          ...
          if ($cant == 0) {
            ...
            INSERT INTO `cmshistorialuno`(
             ...
             `fech_recepcion`,
              ...
              '".trim(utf8_decode($factura[$x]['cli_des']))."',
              '".$fec_venc_creada."',
              '".$fec_emis."',
              '".$fcobro."', 
              '".$diascalle."', 
  ...
  public function buscarHistoricouno($desde) {
      $bus = "SELECT * FROM `cmshistorialuno`
...
where co_ven = '".$co_ven."'
 ORDER BY fech_mod DESC LIMIT 1 ";
  ...
  public function facturaRegionFecha($co_vens,$desde,$hasta) {
   ...
   public function getPeriodos($id,$desde,$hasta) {
        $sel ="
      SELECT * FROM `cmsvendedoresactividad` where 1=1";
      ...
      if (!empty($desde)) {
        $sel.= " and desde='".$desde."' and  hasta='".$hasta."'";
      }          
...
ORDER BY `cmshistorialuno`.`co_vende` ASC
   ...
   public function getFacturasEditadasIndividualmente() {
        
        $conn = $this->getConMYSQL(); 
        $sel ="SELECT documento,cobro,recibido,comision,reserva FROM `cmsdocumentoindividuales` where borrado IS NULL";
...
public function getFacturaEditada($documento,$condicion){
        $conn = $this->getConMYSQL(); 
        $sel ="SELECT  * FROM `cmsdocumentoindividuales` where documento = '".$documento."'and borrado is null";
   ...
   public function getFacturasEditadasIndividualmenteDetallado($documento) {
        
        $conn = $this->getConMYSQL(); 
        $sel ="SELECT doc.* FROM `cmsdocumentoindividuales` as doc where doc.documento='".$documento."'";
   ...
   public function getauditoriaDocumento($documento) {
        
        $conn = $this->getConMYSQL(); 
        $sel ="SELECT * FROM `cmslog` WHERE `detalle` LIKE '%#".$documento."%' ORDER BY `id` DESC ";
   ...
   public function setEliminarCambio($id,$documento) {
        $usuario=$_SESSION['user'];      
        $conn = $this->getConMYSQL(); 
        $sel ="UPDATE `cmsdocumentoindividuales` SET  `borrado`= CURRENT_TIME WHERE id='".$id."' and documento='".$documento."' ";
       ...
       if ($rs) {
           $hecho = 1;
                ...
                $log = "Documento <strong>#".$documento."</strong> se elimino el cambio de documento <strong>".$id."</strong> ";
                $this->add_log($usuario,"Elimino",$log);
       }
        return $hecho;
   }
  ...
  return $gerente;
   ...
   public function agregarCambiosComision($datos) { 
      $usuario=$_SESSION['user'];
      ...
      for ($x=0; $x < count($datos); $x++) { 
        ...
        $ndatos = explode(",", $datos[$x]);

        $sqlBus = "SELECT * FROM `cmsdocumentoindividuales` WHERE `documento` = '".$ndatos[0]."' and borrado is null";
        $resultado = mysqli_query($conn,$sqlBus) or die(mysqli_error($conn));

          if (mysqli_num_rows($resultado) == 0) {
            if (!empty($ndatos[0])) {
              $sql="INSERT INTO `cmsdocumentoindividuales`
              (`id`, `documento`, `cobro`,
               `recibido`, `comision`, `fech_reg`,
                `responsable`, `borrado`, `reserva`)
                 VALUES 
                 (null,'".$ndatos[0]."','".$ndatos[1]."',
                  'null','".$ndatos[2]."',CURRENT_TIME,
                  '".$usuario."',null,'".$ndatos[3]."')";
               
                $rs = mysqli_query($conn,$sql) or die(mysqli_error($conn));
                if($rs){
                   $resulados['realizados']++;
                }
                $log = "<strong>Documento #".$ndatos[0]."</strong>";                    

                if ($ndatos[1] != '0000-00-00') { 
                    $cobro = $this->fechaNormalizada($ndatos[1]);
                    $log.= " Agrego la fecha de cobro <strong>". $cobro."</strong>, ";
                }                       

                if (isset($ndatos[2])) {              
                    $log.= " Modifico comision <strong>".$ndatos[2]."</strong>, ";
                }
                        
                if (isset($ndatos[3])) {       
                     $log.= " Modifico reserva <strong>".$ndatos[3]."</strong> ";
                }
                                               


                $this->add_log($usuario,"Agrego",$log);
