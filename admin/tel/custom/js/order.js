		...
		success:function(response) {
			...
			var tr = '<tr id="row'+count+'" class="'+arrayNumber+'">'+			  				
				'<td>'+
					...
					'<select id="productName'+count+'" name="productName[]" class="selectpicker form-control" data-width="400px" data-size="5" onchange="getProductData('+count+')" required>';
					tr+='<option value="0"> Seleccione Producto</option>';
						// console.log(response);
						$.each(response, function(index, value) {
							tr +='<option data-monto="'+value[2]+'" value="'+value[0]+'" id="changeProduct'+value[0]+'">'+value[0]+' | '+value[2]+' BsF. | Stock:'+value[3]+' | '+value[1]+'</option>';
						});
				...
				'<td>'+
                    '<div class="form-group">'+
                        '<select id="unidad'+count+'" name="unidad[]" class="form-control" required onchange="mult('+count+')">'+
			...
			if(tableLength > 0) {							
				...
				$('#productName'+count).selectpicker({liveSearch:true,windowPadding:'bottom'});
			} else {				
		...
		if(productId == "") {
			...
			$.ajax({
				...
				success:function(response) {
					...
					$("#rate"+row).val(response.monto);
					$("#rateValue"+row).val(response.monto);

					$("#quantity"+row).val(1);

					var total = Number(response.monto) * 1;
					total = total.toFixed(2);
					$("#total"+row).val(total);
					$("#totalValue"+row).val(total);

					var tip='<button id="dd'+row+'" type="button" onClick="modal('+row+');" value="'+response.comentario+'" class="btn btn-primary">'+
					'<i class="glyphicon glyphicon-plus-sign"></i></button>';
					
					$("#apl"+row).html(tip);

					$.ajax({
							url: 'php_action/codUni.php',
							type: 'post',
							data: {productId : productId},
							dataType: 'json',
							success:function(response) {
						...
						// console.log(response);
						$.each(response, function(index, value) {
							opt +='<option value="'+value[1]+'|'+value[0]+'">'+value[0]+'</option>';
						});
...
//  /select unidad de empaque
function mult(row = null) {
	if(row) {
		var unidadId = $("#unidad"+row).val();		
		unidadId=unidadId.split('|');
		equivalencia=unidadId[0];

		var cant=$("#quantity"+row).val();
		var prec=$("#rate"+row).val();

		var mult=Number(cant) * Number(equivalencia);

		var total = Number(prec) * Number(mult);
		total = total.toFixed(2);
		$("#total"+row).val(total);
		$("#totalValue"+row).val(total);
...
function des() {
	...
	for(x = 0; x < tableProductLength; x++) {
		...
		count = count.substring(3);
		//alert(count);
		var $productId = $("#productName"+count+" option:selected");
		precio=Number($productId.data("monto"));
		porcentaje = (precio*descuento)/100;
		nprecio=(precio-porcentaje);
		nprecio = nprecio.toFixed(2);
		$("#rate"+count).val(nprecio);
		$("#rateValue"+count).val(nprecio);
		//alert(x);

		var unidadId = $("#unidad"+count).val();		
		unidadId=unidadId.split('|');
		equivalencia=unidadId[0];

		var cant=$("#quantity"+count).val();
		var prec=$("#rate"+count).val();
		...
		$("#totalValue"+count).val(total);
		
	} // /for
	var tableProductLength2 = $("#productTable tbody tr").length;
	var totalSubAmount = 0;
	for(x = 0; x < tableProductLength2; x++) {
		var tr2 = $("#productTable tbody tr")[x];
		var count2 = $(tr2).attr('id');
		count2 = count2.substring(3);

		totalSubAmount = Number(totalSubAmount) + Number($("#total"+count2).val());
	} // /for
