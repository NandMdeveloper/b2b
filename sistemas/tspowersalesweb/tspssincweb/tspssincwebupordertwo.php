...
$json="";
if (isset($_REQUEST['json'])) $json = $_POST;//DATA email
else $json="";
...
if ($TSPar==1){//REQUESTING DATA
    ...
    if ($tsr!="0"){//REQUESTING DATA USER CHECK STATUS
        ...
        foreach ($obj as $indice => $valor) {
            ...
            foreach ($valor as $indice2 => $valor2) {
                ...
                foreach ($valor2 as $indice3 => $valor3) {
                    ...
                    else if ($indice3=="disg") {
                        foreach ($valor3 as $indice4 => $valor4) {
                            //echo "indice odet 4:".$indice4."<hr>";
                            $comma="";
                                foreach ($valor4 as $indice5 => $valor5) {
                                    //echo "indice disg 5:".$indice5." aca odet valor5:".$valor5."<hr>";
                                    $discgvalues =$discgvalues.$comma."'".$valor5."'";
                                    $comma=",";//prirt 2 timetime
                                    }                    
                                    $discgvaluesT =$discgvaluesT.$comma2."(".$discgvalues.",'".$tsid."','".$tsas."')";
                                $comma2=",";//prirt 2 timetime
                                $discgvalues="";                        
                            }
                        }						
        ...
        //--------------------------------------------------------------------
        $table2="wttorder";    
        $select2="OrderIdA,OrderDesc,CustCode, CustRIF, OrderDateDocu,OrderTotalSub,OrderTotal,OrderTotalTax,UserCode, OrderItemSubI, OrderItemSubG, OrderNumberTax,OrderStatus,OrderSincStatus,OrderTSSIT,OrderTSSITas"; 
        $valores=$ocabvaluesT; 
        ...
        //----------------------------------------------------------------------
        $table2="wttorderd";    
        $select2="OrderIdA, UserCode, CustCode, ItemCode,OrderItemDesc, OrderItemQuantity, OrderItemPrec1, OrderItemSub, OrderItemTotal, OrderItemTax,PromDiscDIdIntern, PromDiscPercentage, PromDiscAmount, OrderItemSubI,OrderItemSincStatus,OrderItemTSSIT,OrderItemTSSITas"; 
        $valores=$odetvaluesT; 
        ...
        //----------------------------------------------------------------------
        $table2="wttorderdpdg";    
//        $select2="OrderIdA, PromDiscId, DiscPercentage, DiscAmount, UserCode, CustCode, PromDiscSincStatus, OrderItemTSSIT, OrderItemTSSITas"; 
		$select2="PromDiscId, DiscPercentage, DiscAmount, UserCode, CustCode, OrderIdA, PromDiscSincStatus, OrderItemTSSIT, OrderItemTSSITas"; 
        $valores=$discgvaluesT; 
        $r2=$ObjBdQ->Query_InsertMV($table2,$select2,$valores);//into bd
        //var_dump($r2);
		//----------------------------------------------------------------------
