...
function update_pedidos_detalles($id){
	$query = " UPDATE wttorderd SET  OrderItemSincStatus=2 ";
...
for($j=0;$j<sizeof($arr_pedidos_detalles);$j++){
	...
	$uniCodPrincipal=$arr_pedidos_detalles[$j]['UnitCodeMain'];
	...
	if($insert_d){
		$udp=update_pedidos_detalles($id_d);
		$fecha=date("Y-m-d H:i:s");
